      ...
      if(util::isNaN(V) || util::isNaN(W))  //Check for minimization errors
        {
            ...
            if(klm_num<cf->params.MatchThreshold){     //If matching keylines are below a certain threshold, reestart the estimation
                ...
                printf("\nCamara Frontal: , match threshold low (%d,%d)?\n",new_buf.ef->KNum(),klm_num);
            }else{
                ...
                if(cf->depthCloudAvailable)
                {
                    std::unique_lock<std::mutex> lock(cf->depth_mutex);
                    pcl::PointCloud<pcl::PointXYZI>::Ptr depthCloud1(new pcl::PointCloud<pcl::PointXYZI>(*(cf->depthCloud)));
                    pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr kdTree1(new pcl::KdTreeFLANN<pcl::PointXYZI>());
                    kdTree1->setInputCloud(depthCloud1);
                    lock.unlock();
                    new_buf.depth_match_num = new_buf.ef->directed_matching_depth(depthCloud1,kdTree1);
                }

                COND_TIME_DEBUG(tlist.push_new();)

                if(cf->depthCloudAvailable){
                    new_buf.ef->fuseDepth();
                }else if(cf->params.StereoAvaiable)
                {
