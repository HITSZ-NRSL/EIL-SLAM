...
void edge_tracker::rotate_keylines(TooN::Matrix <3,3> RotF){
    ...
    for(int i=0;i<kn;i++){
        ...
        Vector <3> q=RotF*makeVector(kl[i].p_m.x/zf,kl[i].p_m.y/zf,1); //归一化平面?
        ...
        )
{
    ...
    const double cang_min_edge=cos(min_thr_ang*M_PI/180.0); //角度转弧度
    ...
    Vector <3> p_m3=BackRot*makeVector(k.p_m.x,k.p_m.y,cam_mod.zfm);    //Back-Rotate the NewKeypoint 焦平面
...
int edge_tracker::directed_matching_depth(
        pcl::PointCloud<pcl::PointXYZI>::Ptr depthCloud,
        pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr kdTree
        )
{
    int nmatch=0;
    int pointNums =  depthCloud->points.size();
    for(int i_kn=0;i_kn<kn;i_kn++){
        if(search_match_depth(kl[i_kn],cam_mod,depthCloud,kdTree)<0)
            continue;
        nmatch++;
    }
    return nmatch;
...
int edge_tracker::search_match_depth(KeyLine &k,cam_model &cam0,pcl::PointCloud<pcl::PointXYZI>::Ptr depthCloud,pcl::KdTreeFLANN<pcl::PointXYZI>::Ptr kdTree)
{
    static int counts=0;
    counts++;
    counts%=1000;
    ...
    pcl::PointXYZI ips;
    ips.x = -10*k.p_m.x/cam0.zf.x;
    ips.y = -10*k.p_m.y/cam0.zf.y;
    ips.z = 10;
    pcl::PointXYZHSV ipr;
    ...
    if (pointSearchSqrDis[0] < 0.3 && pointSearchInd.size() == 3)
    {
        ...
        // else if (ipr.s - maxDepth > 0.2 &&(ipr.s - maxDepth <0.4))
        else if (ipr.s - maxDepth > 0.2)
        {
            k.depth_rho = 1/maxDepth;
            k.depth_s_rho=1e-3;
            k.depth_valid =1;
            return 1;
        }
        // else if (ipr.s - minDepth < -0.2&&(ipr.s - minDepth > -0.4))
        else if (ipr.s - minDepth < -0.2)
        {
            k.depth_rho = 1/minDepth;
            k.depth_s_rho=1e-3;
            k.depth_valid =1;
            return 1;
        }
...
void edge_tracker::fuseDepth(){
    for(int i=0;i<kn;i++){
        ...
        if(ikl.depth_valid<0)
            continue;
        ikl.s_rho=sqrt(1.0/(1.0/util::square(ikl.s_rho0)+1.0/util::square(ikl.depth_s_rho)));
        ikl.rho=(ikl.rho0/util::square(ikl.s_rho0)+ikl.depth_rho/util::square(ikl.depth_s_rho))*util::square(ikl.s_rho);
        //ikl.s_rho=ikl.s_rho0;
    }
}
                                       ...
                                       bool re_escale)
{
    ...
    if(re_escale){                      //Optionally rescale
        for(int ikl=0;ikl<kn;ikl++){
           kl[ikl].rho/=Kp;
           kl[ikl].s_rho/=Kp;
        }
