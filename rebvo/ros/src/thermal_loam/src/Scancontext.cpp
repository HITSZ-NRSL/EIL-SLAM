...
std::vector<std::pair<int, float>> SCManager::RelocalizationID ( pcl::PointCloud<SCPointType> & _scan_down  )
{
    int loop_id { -1 }; // init with -1, -1 means no Relocalization (== LeGO-LOAM's variable "closestHistoryFrameID")

    auto curr_desc = makeScancontext(_scan_down); // current observation (query)
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext( curr_desc ); //Ring向量(20*1);
    auto curr_key = eig2stdvec( ringkey );// current observation (query)

    std::vector<std::pair<int, float>> res;
    if( polarcontext_invkeys_mat_.size()<=1)
    {
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        // std::cout<<"cur candidate: "<<candidate_indexes[candidate_iter_idx]<<","<<candidate_dist<<std::endl;


        if( candidate_dist < SC_DIST_THRES )
        {
            float yaw_diff_rad = deg2rad(candidate_align * PC_UNIT_SECTORANGLE);
            std::pair<int, float> result {candidate_indexes[candidate_iter_idx], yaw_diff_rad};
            res.push_back(result);
    ...
    //     // cout << "[Relocalization found] Nearest distance: " << min_dist << " btn " << polarcontexts_.size()-1 << " and " << nn_idx << "." << endl;
    //     // cout << "[Relocalization found] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    // }
    // else
    // {
    //     std::cout.precision(3); 
    //     // cout << "[Not Relocalization] Nearest distance: " << min_dist << " btn " << polarcontexts_.size()-1 << " and " << nn_idx << "." << endl;
    //     // cout << "[Not Relocalization] yaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl;
    // }
...
std::vector<std::pair<double,int>> SCManager::detectLoopClosureToPID ( void )
{
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        if( candidate_dist < SC_DIST_THRES ) 
        {
            candidates.push_back(std::make_pair(candidate_dist,candidate_indexes[candidate_iter_idx]));
        }
    }
    std::sort(candidates.begin(),candidates.end(),judge);
    t_calc_dist.toc();
...
std::pair<int, float> SCManager::RelocalizationID ( pcl::PointCloud<SCPointType> & _scan_down,bool flag  )
{
    int loop_id { -1 }; // init with -1, -1 means no Relocalization (== LeGO-LOAM's variable "closestHistoryFrameID")
