...
#include "vloam/nanoflann.hpp"
#include "KDTreeVectorOfVectorsAdaptor.h"

#include "vloam/tic_toc.h"
...
public: 
    ...
    std::vector<std::pair<int, float>> RelocalizationID( pcl::PointCloud<SCPointType> & _scan_down  ); // int: nearest node index, float: relative yaw  
    std::pair<int, float> RelocalizationID( pcl::PointCloud<SCPointType> & _scan_down,bool flag  ); // int: nearest node index, float: relative yaw  
    std::vector<std::pair<double,int>> detectLoopClosureToPID( void ); // int: nearest node index, float: relative yaw  
public:
    // hyper parameters ()
    ...
    const double SC_DIST_THRES = 0.5;
    // config 
