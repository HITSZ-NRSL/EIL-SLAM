...
protected:
    ...
    int max_img_value;              //Maximum value of intensity in the images
...
public:
    ...
    ~edge_finder();
    void detect(Image<DetectorImgType> &originImg,sspace *ss ,int plane_fit_size,double pos_neg_thresh,double dog_thresh,
                int kl_max, double &tresh, int &l_kl_num, int kl_ref=0, double gain=0, double thresh_max=1e10, double thresh_min=1-10);
    void build_mask(Image<DetectorImgType> &originImg,sspace *ss, int kl_max, int win_s, float per_hist, float grad_thesh, float dog_thesh);
